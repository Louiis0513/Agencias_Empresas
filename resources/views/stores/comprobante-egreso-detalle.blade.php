            ...
            <div class="flex items-center gap-3">
                @if(!$comprobante->isReversed())
                    <a href="{{ route('stores.comprobantes-egreso.edit', [$store, $comprobante]) }}"
                       class="px-4 py-2 bg-brand text-white rounded-xl shadow-[0_0_15px_rgba(34,114,255,0.3)] hover:shadow-[0_0_20px_rgba(34,114,255,0.4)] text-sm font-medium">
                        Editar comprobante
                    </a>
                    <button type="button"
                    ...
                    <form method="POST" action="{{ route('stores.comprobantes-egreso.anular', [$store, $comprobante]) }}" id="form-anular">
                        ...
                        <div class="space-y-2 mb-4" id="origenes-reverso-container">
                            ...
                            <div class="origen-reverso-row flex gap-2 items-center">
                                ...
                                <input type="number" name="origenes[{{ $i }}][amount]" step="0.01" min="0.01" value="{{ $o->amount }}" class="w-24 rounded-md border-white/10 bg-white/5 text-gray-100 text-sm origen-amount" required placeholder="Monto">
                ...
                <div class="p-6">
                    <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-100">{{ $comprobante->number }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-400">Fecha</p>
                            <p class="text-lg font-semibold text-gray-100">{{ $comprobante->payment_date->format('d/m/Y') }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-lg font-semibold text-gray-100">{{ $comprobante->beneficiary_name ?? '—' }}</p>
                        </div>
                        @if($comprobante->proveedor)
                            <div>
                                <p class="text-sm text-gray-400">Proveedor</p>
                                <p class="text-lg font-semibold text-gray-100">{{ $comprobante->proveedor->nombre }}</p>
                            </div>
                        @endif
                        <div>
                        ...
                        <div>
                            <p class="text-sm text-gray-400">Registrado por</p>
                            <p class="text-lg font-semibold text-gray-100">{{ $comprobante->user->name ?? '—' }}</p>
                        </div>
                        @if($comprobante->notes)
                            <div class="md:col-span-2">
                                <p class="text-sm text-gray-400">Notas</p>
                                <p class="text-gray-100">{{ $comprobante->notes }}</p>
                            </div>
                        @endif
                        @if($comprobante->isReversed())
                            <div class="md:col-span-2">
                    ...
                    <div class="mb-6">
                        <h3 class="text-sm font-medium text-gray-100 mb-2">Destinos (a qué se destinó el dinero)</h3>
                        <table class="min-w-full divide-y divide-white/5">
                            <thead class="border-b border-white/5">
                                <tr>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Tipo</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Detalle</th>
                    ...
                    <div class="mb-6">
                        <h3 class="text-sm font-medium text-gray-100 mb-2">Orígenes (de qué bolsillos salió)</h3>
                        <table class="min-w-full divide-y divide-white/5">
                            <thead class="border-b border-white/5">
                                <tr>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Bolsillo</th>
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-400">Referencia (cheque/transacción)</th>
                                    <th class="px-3 py-2 text-right text-xs font-medium text-gray-400">Monto</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-white/5">
            ...
            return {
                ...
                bindOrigenesReverso() {
                    ...
                    addBtn.onclick = () => {
                        ...
                        row.innerHTML = `
                            <select name="origenes[${idx}][bolsillo_id]" class="flex-1 rounded-md border-white/10 bg-white/5 text-gray-100 text-sm" required>
                                <option value="">Seleccionar bolsillo</option>
                                ${bolsillos.map(b => `<option value="${b.id}">${b.name} (${parseFloat(b.saldo).toFixed(2)})</option>`).join('')}
                            </select>
                            <input type="text" name="origenes[${idx}][reference]" class="w-28 rounded-md border-white/10 bg-white/5 text-gray-100 text-sm" placeholder="Ref.">
                            <input type="number" name="origenes[${idx}][amount]" step="0.01" min="0.01" class="w-24 rounded-md border-white/10 bg-white/5 text-gray-100 text-sm origen-amount" required placeholder="Monto">
