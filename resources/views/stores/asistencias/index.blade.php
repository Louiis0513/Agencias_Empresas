        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex items-center gap-3">
                ...
                <button type="button" x-show="showForm" x-on:click="showForm = false"
                    class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-white/5 transition">
                    Cerrar
                </button>
            </div>
                    ...
                    <form method="GET" action="{{ route('stores.asistencias', $store) }}" class="mb-6 flex flex-wrap items-end gap-4">
                        <div>
                            ...
                            <input type="date" name="from" id="from" value="{{ $from?->format('Y-m-d') }}"
                                class="rounded-md border-white/10 bg-white/5 text-gray-100 text-sm">
                        </div>
                        <div>
                        ...
                        <div>
                            <label for="customer_id" class="block text-xs font-medium text-gray-400 uppercase mb-1">ID Cliente</label>
                            <input type="number" name="customer_id" id="customer_id" value="{{ $customerId }}"
                                placeholder="Opcional" min="1"
                                class="rounded-md border-white/10 bg-white/5 text-gray-100 text-sm w-28">
                        </div>
                        <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-md">
                            Filtrar
                        </button>
                        <a href="{{ route('stores.asistencias', $store) }}" class="text-sm text-gray-400 hover:text-brand transition">
                            Limpiar
                        </a>
                    </form>
