<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('stores.invoices', $store) }}" class="text-sm text-gray-400 hover:text-brand transition">
                ← Volver a Facturas
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-dark-card border border-white/5 overflow-hidden sm:rounded-xl">
                <div class="p-6">
                    ...
                    <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <p class="text-sm text-gray-400">Número de Factura</p>
                            <p class="text-lg font-semibold text-gray-100">#{{ $invoice->id }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-400">Fecha de Emisión</p>
                            <p class="text-lg font-semibold text-gray-100">{{ $invoice->created_at->format('d/m/Y H:i') }}</p>
                        </div>
                        <div>
                    ...
                    <div class="mb-6 border-t border-gray-200 dark:border-gray-700 pt-4">
                        <h3 class="text-sm font-medium text-gray-100 mb-2">Información del Cliente</h3>
                            ...
                            @if($invoice->customer)
                                @if($invoice->customer->email)
                                    <div>
                                        ...
                                        <p class="text-base text-gray-100">{{ $invoice->customer->email }}</p>
                                    </div>
                                @endif
                                @if($invoice->customer->phone)
                                    <div>
                                        <p class="text-sm text-gray-400">Teléfono</p>
                                        <p class="text-base text-gray-100">{{ $invoice->customer->phone }}</p>
                                    </div>
                                @endif
                                @if($invoice->customer->document_number)
                                    <div>
                                        <p class="text-sm text-gray-400">Documento</p>
                                        <p class="text-base text-gray-100">{{ $invoice->customer->document_number }}</p>
                                    </div>
                                @endif
                                @if($invoice->customer->address)
                                    <div class="md:col-span-2">
                                        <p class="text-sm text-gray-400">Dirección</p>
                                        <p class="text-base text-gray-100">{{ $invoice->customer->address }}</p>
                                    </div>
                                @endif
                            @endif
                        </div>
                    ...
                    <div class="mb-6 border-t border-gray-200 dark:border-gray-700 pt-4">
                        <h3 class="text-sm font-medium text-gray-100 mb-2">Información Adicional</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                            ...
                            <div>
                                <p class="text-sm text-gray-400">Tienda</p>
                                <p class="text-base text-gray-100">{{ $store->name }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-400">Última Actualización</p>
                                <p class="text-base text-gray-100">{{ $invoice->updated_at->format('d/m/Y H:i') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        <h3 class="text-lg font-semibold text-gray-100 mb-4">Detalles</h3>
                    ...
                    <div class="mt-6 border-t border-gray-200 dark:border-gray-700 pt-4 space-y-4">
                        ...
                        @if($comprobantes->isNotEmpty())
                            <div>
                                <h3 class="text-sm font-medium text-gray-100 mb-2">Comprobante(s) de ingreso</h3>
                                <ul class="space-y-1">
