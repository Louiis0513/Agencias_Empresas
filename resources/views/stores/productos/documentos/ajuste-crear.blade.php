        ...
        <div class="flex-1 space-y-4 px-3 py-4 sm:px-5 lg:px-6 pb-28">
            ...
            <template x-if="!errorMsg">
                <div class="rounded-lg border border-sky-500/30 bg-sky-950/20 px-3 py-2 text-sm text-sky-100">
                    Al contabilizar se crea el documento A, los movimientos de inventario y el asiento (inventario vs cuenta de ingreso/gasto).
                </div>
            </template>

            <div class="space-y-3 rounded-xl border border-white/5 bg-dark-card p-4 sm:p-5">
                <p class="text-sm text-gray-400">
                    Registra correcciones de cantidad o valor. Por cada línea indica si aumenta o disminuye y la cuenta contable de contrapartida.
                </p>
                <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-6">
                    <div class="w-full sm:w-56">
                        <label class="mb-1 block text-sm text-gray-400">Fecha de elaboración <span class="text-red-400">*</span></label>
                    ...
                    <table class="w-full min-w-[1280px] text-sm">
                        <thead>
                            <tr class="bg-sky-900/90 text-left text-xs font-semibold uppercase tracking-wide text-white">
                                ...
                                <th class="min-w-[10rem] px-3 py-3" x-show="manejaBodegas">Bodega</th>
                                <th class="w-36 px-3 py-3">Aumenta / Disminuye</th>
                                <th class="min-w-[11rem] px-3 py-3">Centro de costo</th>
                                <th class="w-28 px-3 py-3">Cantidad</th>
                                <th class="w-36 px-3 py-3">Costo unitario</th>
                                <th class="min-w-[14rem] px-3 py-3">Cuenta Ingreso / Gasto</th>
                                <th class="w-32 px-3 py-3">Costo total</th>
                        ...
                        <tbody>
                            <template x-for="(line, index) in lines" :key="line.key">
                                <tr class="border-b border-white/5 align-middle text-gray-200">
                                    ...
                                    <td class="px-3 py-2.5">
                                        <select x-model="line.direccion"
                                                @change="ensureTrailingEmptyLine()"
                                                class="w-full rounded-lg border border-white/15 bg-white/5 py-2 text-sm text-gray-100 focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                                    ...
                                    <td class="px-3 py-2.5">
                                        <div class="relative" :data-dd-anchor="'cuenta-' + line.key">
                                            <input type="text"
                                                   x-model="line.cuenta_search"
                                                   @focus="openDd(line, 'cuenta', $event)"
                                                   @click="openDd(line, 'cuenta', $event)"
                                                   @input="openDd(line, 'cuenta', $event); onCuentaSearchInput(line)"
                                                   @keydown.escape.stop="closeDd()"
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                ...
                <div class="flex items-stretch rounded-xl border border-white/5 bg-dark-card p-4">
                    <div class="flex w-full flex-col justify-center rounded-lg border border-white/10 bg-white/[0.03] px-4 py-3">
                        <div class="text-sm text-gray-400">Valor total ajuste:</div>
            ...
            <div class="flex items-center justify-between gap-4 px-4 py-3 sm:px-6">
                ...
                <div class="flex flex-col items-end gap-1">
                    ...
                    <p class="text-[11px] text-gray-500" x-show="!puedeContabilizar && !saving">
                        Completa fecha, productos, dirección, cuenta, cantidades y costos mayores a cero.
                    </p>
                </div>
            </div>
        </div>
