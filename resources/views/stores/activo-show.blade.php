            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex flex-wrap justify-between items-start gap-4">
                        <div>
                            <h3 class="text-lg font-medium text-gray-100">{{ $activo->name }}</h3>
                            <p class="text-sm text-gray-400 mt-1">Serial: {{ $activo->serial_number }} @if($activo->code)— {{ $activo->code }}@endif</p>
                        </div>
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h4 class="text-sm font-medium text-gray-400 uppercase mb-2">Datos del activo</h4>
                        <dl class="space-y-2 text-sm">
                            <div><dt class="text-gray-400 inline">Marca / Modelo:</dt> <dd class="inline text-gray-100">{{ $activo->brand ?? '-' }} {{ $activo->model ? " / {$activo->model}" : '' }}</dd></div>
                            <div><dt class="text-gray-400 inline">Condición:</dt> <dd class="inline text-gray-100">{{ $activo->condition ? (\App\Models\Activo::condicionesDisponibles()[$activo->condition] ?? $activo->condition) : '-' }}</dd></div>
                            <div><dt class="text-gray-400 inline">Estado:</dt> <dd class="inline"><span class="px-2 py-0.5 text-xs font-medium rounded-full
                            ...
                            @if($activo->description)
                                <div><dt class="text-gray-400 block">Descripción:</dt> <dd class="text-gray-100">{{ $activo->description }}</dd></div>
                            @endif
                        </dl>
                    </div>
                    <div>
                        <h4 class="text-sm font-medium text-gray-400 uppercase mb-2">Ubicación y responsable</h4>
                        <dl class="space-y-2 text-sm">
                            <div><dt class="text-gray-400 inline">Ubicación:</dt> <dd class="inline text-gray-100">{{ $activo->locationRelation?->name ?? $activo->location ?? '-' }}</dd></div>
                            <div><dt class="text-gray-400 inline">Asignado a:</dt> <dd class="inline text-gray-100">{{ $activo->assignedTo?->name ?? '-' }}</dd></div>
                        </dl>
                    </div>
                </div>

                <div class="px-6 pb-6">
                    <h4 class="text-sm font-medium text-gray-400 uppercase mb-3">Historial de vida</h4>
                            ...
                            <table class="min-w-full divide-y divide-white/5 text-sm">
                                <thead class="border-b border-white/5">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-400 uppercase">Descripción</th>
                                        <th class="px-3 py-2 text-left text-xs font-medium text-gray-400 uppercase">Usuario</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-white/5">
                                    @foreach($activo->movimientos as $mov)
                                        <tr>
                                            <td class="px-3 py-2 text-gray-100">{{ $mov->created_at->format('d/m/Y H:i') }}</td>
                                            ...
                                            <td class="px-3 py-2 text-gray-100">{{ $mov->description ?? '-' }}</td>
