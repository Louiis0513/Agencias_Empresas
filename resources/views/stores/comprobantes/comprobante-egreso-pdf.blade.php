...
<head>
    ...
    <title>{{ $tituloDocumento }} {{ $comprobante->number }}</title>
    <style>
...
@if($c->isReversed())
    <div class="warn">{{ __('Este comprobante fue anulado o revertido y no tiene efecto contable.') }}</div>
@endif
...
<table class="w-100" style="margin-bottom: 6px;"><tr>
    ...
    <td class="box w-50">
        <p class="title-blue" style="font-size: 12pt;">{{ $tituloDocumento }}</p>
        ...
        <table class="data" style="margin: 0;">
            ...
            <tbody>
                @foreach($c->origenes as $o)
                    <tr>
                        <td>{{ $o->bolsillo->name ?? '—' }}</td>
                        <td>{{ filled($o->reference) ? $o->reference : '—' }}</td>
                        <td class="num">{{ money((float) $o->amount, $cur) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
...
<p class="muted" style="margin-top: 6px; font-size: 7.5pt;">{{ __('Registrado por') }}: {{ $c->user->name ?? '—' }}</p>
