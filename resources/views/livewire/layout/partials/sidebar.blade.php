@php
    ...
    $canProductos = $store && ($perm->can($store, 'products.view') || $perm->can($store, 'categories.view') || $perm->can($store, 'attribute-groups.view') || $perm->can($store, 'inventario.view') || $perm->can($store, 'proveedores.view') || $perm->can($store, 'product-purchases.view'));
...
@endphp
{{-- Sidebar: colapsado = solo iconos; expandido = icono + texto. Hover expande cuando está colapsado. --}}
<aside
    :class="sidebarCollapsed ? 'w-[72px]' : 'w-64'"
    @mouseenter="if(sidebarCollapsed) sidebarCollapsed = false"
    @mouseleave="if(sidebarCollapsed) sidebarCollapsed = true"
    class="fixed left-0 top-0 z-40 h-screen flex flex-col bg-dark-card border-r border-white/5 transition-all duration-300 ease-in-out -translate-x-full lg:translate-x-0"
    :class="{ 'translate-x-0': sidebarMobileOpen }"
>
    {{-- Logo + toggle --}}
    <div class="flex h-16 shrink-0 items-center justify-between border-b border-white/5 px-3">
        <a href="{{ route('dashboard') }}" wire:navigate class="flex items-center gap-2 min-w-0">
            ...
            <span x-show="!sidebarCollapsed" x-transition class="whitespace-nowrap font-semibold text-white truncate">{{ config('app.name') }}</span>
        </a>
        <button type="button" @click="sidebarCollapsed = !sidebarCollapsed" x-show="!sidebarCollapsed" class="shrink-0 rounded-lg p-1.5 text-gray-400 hover:bg-white/5 hover:text-white">
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden py-4" @click="if ($event.target.closest('a')) sidebarMobileOpen = false">
        <div class="px-3 text-xs font-semibold uppercase tracking-wider text-gray-500" x-show="!sidebarCollapsed">Menú</div>
            ...
            @if($store)
                ...
                <li>
                    <a href="{{ route('stores.dashboard', $store) }}" wire:navigate class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-gray-300 transition {{ request()->routeIs('stores.dashboard') ? 'bg-brand/20 text-brand' : 'hover:bg-white/5 hover:text-white' }}">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="whitespace-nowrap">Resumen</span>
                    </a>
                </li>
                {{-- Personas (dropdown) --}}
                @if($canPersonas)
                <li x-data="{ open: {{ $inPersonas ? 'true' : 'false' }} }">
                    <button type="button" @click="open = !open" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-gray-300 transition {{ $inPersonas ? 'bg-brand/20 text-brand' : 'hover:bg-white/5 hover:text-white' }}">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="flex-1 whitespace-nowrap">Personas</span>
                        <svg x-show="!sidebarCollapsed" :class="open && 'rotate-180'" class="h-4 w-4 shrink-0 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    <div x-show="open && !sidebarCollapsed" x-transition class="ml-4 mt-0.5 space-y-0.5 border-l border-white/5 pl-2">
                ...
                <li x-data="{ open: {{ $inProductos ? 'true' : 'false' }} }">
                    <button type="button" @click="open = !open" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-gray-300 transition {{ $inProductos ? 'bg-brand/20 text-brand' : 'hover:bg-white/5 hover:text-white' }}">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="flex-1 whitespace-nowrap">Productos</span>
                        <svg x-show="!sidebarCollapsed" :class="open && 'rotate-180'" class="h-4 w-4 shrink-0 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    <div x-show="open && !sidebarCollapsed" x-transition class="ml-4 mt-0.5 space-y-0.5 border-l border-white/5 pl-2">
                ...
                <li x-data="{ open: {{ $inFinanciero ? 'true' : 'false' }} }">
                    <button type="button" @click="open = !open" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-gray-300 transition {{ $inFinanciero ? 'bg-brand/20 text-brand' : 'hover:bg-white/5 hover:text-white' }}">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="flex-1 whitespace-nowrap">Financiero</span>
                        <svg x-show="!sidebarCollapsed" :class="open && 'rotate-180'" class="h-4 w-4 shrink-0 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    <div x-show="open && !sidebarCollapsed" x-transition class="ml-4 mt-0.5 space-y-0.5 border-l border-white/5 pl-2">
                ...
                <li x-data="{ open: {{ $inVentas ? 'true' : 'false' }} }">
                    <button type="button" @click="open = !open" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-gray-300 transition {{ $inVentas ? 'bg-brand/20 text-brand' : 'hover:bg-white/5 hover:text-white' }}">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="flex-1 whitespace-nowrap">Ventas</span>
                        <svg x-show="!sidebarCollapsed" :class="open && 'rotate-180'" class="h-4 w-4 shrink-0 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    <div x-show="open && !sidebarCollapsed" x-transition class="ml-4 mt-0.5 space-y-0.5 border-l border-white/5 pl-2">
                ...
                <li x-data="{ open: {{ $inSuscripciones ? 'true' : 'false' }} }">
                    <button type="button" @click="open = !open" class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-gray-300 transition {{ $inSuscripciones ? 'bg-brand/20 text-brand' : 'hover:bg-white/5 hover:text-white' }}">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="flex-1 whitespace-nowrap">Suscripciones</span>
                        <svg x-show="!sidebarCollapsed" :class="open && 'rotate-180'" class="h-4 w-4 shrink-0 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    <div x-show="open && !sidebarCollapsed" x-transition class="ml-4 mt-0.5 space-y-0.5 border-l border-white/5 pl-2">
                ...
                <li class="border-t border-white/5 pt-2 mt-2">
                    <a href="{{ route('dashboard') }}" wire:navigate class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-gray-400 hover:bg-red-500/10 hover:text-red-400 transition">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="whitespace-nowrap">Salir de la tienda</span>
                    </a>
                </li>
            @else
                ...
                <li>
                    <a href="{{ route('dashboard') }}" wire:navigate class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-gray-300 transition {{ request()->routeIs('dashboard') ? 'bg-brand/20 text-brand' : 'hover:bg-white/5 hover:text-white' }}">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition class="whitespace-nowrap">Mis Tiendas</span>
                    </a>
                </li>
            @endif
        </ul>
